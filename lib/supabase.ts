...
const _supabase: SupabaseClient | any = (supabaseUrl && supabaseAnonKey)
  ? createClient(supabaseUrl, supabaseAnonKey)
  : {
      from: (_table: string) => ({
        select: (_sel?: string) => ({
          order: (_col: string, _opts?: any) => Promise.resolve({ data: null, error: new Error('Supabase not configured') }),
        }),
      }),
    }
